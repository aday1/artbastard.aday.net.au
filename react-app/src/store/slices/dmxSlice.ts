...
export const createDmxSlice = (
  ...
  setDmxChannel: (channel, value, sendToBackend = true) => {
    ...
    if (sendToBackend) {
      console.log(`[DMX Store] Queueing backend DMX update: channel=${channel}, value=${clampedValue}`);
  ...
  setMultipleDmxChannels: (updates, sendToBackend = true) => {
    ...
    if (sendToBackend) {
      console.log('[DMX Store] setMultipleDmxChannels: Queueing backend DMX batch:', updates);
