...
import { APC40_DEVICE_KNOB_ASSIGNMENTS } from './apc40Metadata';
...
const buildApc40SuperControlMappings = (): SuperControlBinding[] => {
  ...
  APC40_DEVICE_KNOB_ASSIGNMENTS.forEach(({ slot, cc, controlName, roleLabel }) => {
    bindings.push({
      controlName,
      channel: 0,
      controller: cc,
      label: `Device Knob ${slot + 1} -> ${roleLabel}`,
    });
  });
