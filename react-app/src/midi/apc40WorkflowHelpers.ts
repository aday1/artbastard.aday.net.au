...
export function resolveApc40DeviceRoleSlots(
  ...
  selected.forEach((fixture) => {
    fixture.channels.forEach((channel) => availableTypes.add(normalizeChannelType(channel.type)));
  });

  const matching = DEVICE_ROLE_PRIORITY.filter((slot) =>
    slot.aliases.some((alias) => availableTypes.has(normalizeChannelType(alias)))
  );
...
export function buildRoleUpdates(
  ...
  targets.forEach((fixture) => {
    const channelIndex = fixture.channels.findIndex((channel) => aliases.has(normalizeChannelType(channel.type)));
    if (channelIndex >= 0) {
