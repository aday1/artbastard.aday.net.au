...
describe('APC40 workflow helpers', () => {
  ...
  it('uses channel names when fixture profile types are generic', () => {
    const fixtures: Fixture[] = [{
      id: 'generic-spot',
      name: 'Generic Spot',
      type: 'moving-head',
      startAddress: 101,
      channels: [
        { name: 'Dimmer', type: 'channel' },
        { name: 'Gobo Wheel', type: 'channel' },
        { name: 'Color Wheel', type: 'channel' },
      ],
    }];

    const roles = resolveApc40DeviceRoleSlots(fixtures, ['generic-spot']);
    expect(roles.slice(0, 2).map((role) => role.controlName)).toEqual(['gobo', 'color_wheel']);

    expect(buildRoleUpdates(fixtures, ['generic-spot'], roles[0], 200)).toEqual({ 101: 200 });
  });
